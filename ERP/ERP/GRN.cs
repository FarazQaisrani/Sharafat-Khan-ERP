...
namespace ERP
{
    public partial class GRN : Form
    {
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (dr.Read())
            {
                textBox3.Text = "GRN_" +  dr["POID"].ToString();
        ...
        private void GRN_Load(object sender, EventArgs e)
        {
            ...
            cmd = new OleDbCommand("select POID from PO where Status = 'Open' ", mc.conn);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                comboBox2.Items.Add(dr["POID"]);
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            cmd = new OleDbCommand("insert into GRN (GRNID , BaseDocument , Status , VName , DDate , GRDate) values (@GRNID , @BaseDocument , @Status , @VName , @DDate , @GRDate)", mc.conn);
            ...
            cmd.Parameters.AddWithValue("@GRDate" , dateTimePicker1.Value);
            cmd.ExecuteNonQuery();


            cmd = new OleDbCommand("Update PO set Status = 'Close', GoodRecieved = 'Yes' where POID = '" + comboBox2.Text + "' ", mc.conn);
            cmd.ExecuteNonQuery();
